...
def is_key_fringe(red: int, green: int, blue: int, key: str) -> bool:
    if key == "green":
        return green > 92 and green > red * 1.25 and green > blue * 1.25
    if key == "magenta":
        return red > 92 and blue > 92 and green < min(red, blue) * 0.72
    raise ValueError(f"Unsupported key: {key}")


def remove_small_components(image: Image.Image, minimum_area: int = 180) -> Image.Image:
    pixels = image.load()
...
camera_flash = fit_subject(
    "camera_flash_transparent.png",
    key="green",
    target_extent=60,
)
journalism = fit_subject(
    "journalism_transparent.png",
    key="green",
    target_extent=58,
    rotation_degrees=-10,
)
cat_scratch = fit_subject(
    "cat_scratch_transparent.png",
    key="magenta",
    target_extent=60,
)

outputs = {
    "ability_camera_flash.png": camera_flash,
    "ability_journalism.png": journalism,
    "ability_cat_scratch.png": cat_scratch,
}
