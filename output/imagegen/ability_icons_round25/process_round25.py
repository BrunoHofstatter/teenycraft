...
def fit_subject(
    ...
    # The chroma helper produces a soft matte at source resolution. Use a
    # binary alpha mask before nearest-neighbor reduction so the production
    # sprite has deliberately crisp Minecraft-style edges.
...
outputs = {
    "ability_stinky_fish.png": fit_subject(
        "stinky_fish_transparent_source.png",
        target_extent=58,
    ),
    "ability_deadly_kiss.png": fit_subject(
        "kiss_symbol_transparent_source.png",
        target_extent=38,
    ),
    "ability_darwin.png": fit_subject(
        "darwin_transparent_source.png",
        target_extent=58,
    ),
}

# Deadly Kiss and Puddin' Pucker intentionally share the approved symbol.
outputs["ability_puddin_pucker.png"] = outputs["ability_deadly_kiss.png"].copy()

for filename, image in outputs.items():
    image.save(ROOT / filename, optimize=True)

for stem, image in (
    ("stinky_fish", outputs["ability_stinky_fish.png"]),
    ("kiss_symbol", outputs["ability_deadly_kiss.png"]),
    ("darwin", outputs["ability_darwin.png"]),
):
...
qa_sheet(
    [
        ("Stinky Fish", outputs["ability_stinky_fish.png"]),
        ("Kiss symbol", outputs["ability_deadly_kiss.png"]),
        ("Darwin", outputs["ability_darwin.png"]),
    ]
)
