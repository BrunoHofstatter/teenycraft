...
def extract_burp_source() -> None:
    """Remove the magenta field without desaturating Silkie's warm pinks."""
    source = Image.open(ROOT / "burp_surprise_source.png").convert("RGBA")
    output = source.copy()
    pixels = []
    for red, green, blue, _ in source.get_flattened_data():
        ...
        pixels.append((red, green, blue, 0 if is_chroma else 255))
    output.putdata(pixels)
    output.save(ROOT / "burp_surprise_transparent_source.png", optimize=True)
...
def fit_subject(
    ...
    # Production icons use crisp binary alpha before nearest-neighbor scaling.
...
def remove_magenta_fringe(image: Image.Image) -> Image.Image:
    """Drop isolated purple edge pixels left by Silkie's similar chroma key."""
    result = image.copy()
    for y in range(SIZE):
        for x in range(SIZE):
...
outputs = {
    # The complete binary halo uses most of the canvas, while the controller
    # remains deliberately smaller at the center per the user's direction.
    "ability_robo_buddy.png": fit_subject(
        "robo_buddy_transparent_source.png",
        target_extent=58,
    ),
    "ability_burp_surprise.png": fit_subject(
        "burp_surprise_transparent_source.png",
        target_extent=58,
    ),
    "ability_battery_drain__variant_eel.png": fit_subject(
        "battery_drain_eel_transparent_source.png",
        target_extent=60,
    ),
}

outputs["ability_burp_surprise.png"] = remove_magenta_fringe(
    outputs["ability_burp_surprise.png"]
)
...
for stem, filename in (
    ("robo_buddy", "ability_robo_buddy.png"),
    ("burp_surprise", "ability_burp_surprise.png"),
    ("battery_drain_eel", "ability_battery_drain__variant_eel.png"),
):
...
qa_sheet(
    [
        ("Robo Buddy", outputs["ability_robo_buddy.png"]),
        ("Burp Surprise", outputs["ability_burp_surprise.png"]),
        ("Battery Drain Eel", outputs["ability_battery_drain__variant_eel.png"]),
    ]
)
...
for filename, image in outputs.items():
    ...
    print(
        ...
        f"visible_chroma={visible_chroma}"
    )
