...
public class NPCTeamLoader extends SimplePreparableReloadListener<Map<String, NPCTeamLoader.NPCTeamDefinition>> {
    ...
    protected Map<String, NPCTeamDefinition> prepare(ResourceManager resourceManager, ProfilerFiller profiler) {
        ...
        for (Map.Entry<ResourceLocation, Resource> entry : resources.entrySet()) {
            try (Reader reader = new InputStreamReader(entry.getValue().open())) {
                ...
                BattleAiProfile aiProfile = parseAiProfile(json);
    ...
    static BattleAiProfile parseAiProfile(JsonObject teamJson) {
        if (!teamJson.has("ai") || !teamJson.get("ai").isJsonObject()) {
            return BattleAiProfile.DEFAULT;
        }

        JsonObject aiJson = teamJson.getAsJsonObject("ai");
        ...
        return new BattleAiProfile(
                ...
                aiJson.has("enabled") ? aiJson.get("enabled").getAsBoolean() : preset.enabled()
        );
    }
}
